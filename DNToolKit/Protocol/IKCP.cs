...
// Technically unnecessary to keep the entire kcp file since like about 80% of the code here is for sending packets
...
class IKCP
{
    ...
    public static ushort ikcp_decode16u(byte[] p, ref int offset)
    {
        int pos = offset;
        offset += 2;
        return (ushort)(p[pos] | (ushort)(p[pos + 1] << 8));
    }
    ...
    internal class Segment
    {
        ...
        internal void Encode(byte[] ptr, ref int offset)
        {
            ...
            ikcp_encode16u(ptr, offset + 10, (ushort)wnd);
    ...
    readonly uint conv_ = 0;
    readonly uint token_ = 0;
    uint mtu_ = 0;
    uint mss_ = 0;
    uint state_ = 0;
    ...
    public IKCP(uint conv, uint token, object user)
    {
    ...
    public int Input(byte[] data, int offset, int size)
    {
        ...
        while (true)
        {
            ...
            uint token = ikcp_decode32u(data, ref offset);
    ...
    // rather than returning current + 0 or current + minimal
    // only to subtract it later
    // returns minimal/0 
